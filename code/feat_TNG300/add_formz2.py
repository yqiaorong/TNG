...
print('')
print(f'>>> Add formation time (old method) to halo data <<<')
print('\nInput arguments:')
...
for fname in halos_fnames:
    ...
    for current_mass, subhalo_id in tqdm(
        ...
        if result is None:
            ...
            for s, gr_id in zip(result['SnapNum'], result['SubhaloGrNr']):
                ...
                if halo_info == 0:
                    result['mass'].append(np.nan)
                else:
                    result['mass'].append(halo_info/data['h']) # Physical mass!
            
            # Find the index in result that mass is closest to the half of current mass
            mass = np.array(result['mass'])
            mask = (mass >= current_mass*0.4) & (mass <= current_mass*0.6)
            if not np.any(mask):
                formz.append([np.nan])
            else:
                closest_idx = np.argmin(np.abs(mass[mask] - current_mass/2))
    ...
    if rank == 0:
        ...
        data['formzOLD'] = formz_full.squeeze()
