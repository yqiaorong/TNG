...
func_list = [func0, func1, func2, func3, func4]
...
for ifix_z, fix_z in enumerate(uniq_z):
    ...
    axs[0].plot(logM_th, y_th, c=f'C{ifix_z}', # c=cmap1(ifix_z/len(uniq_z)), 
                label=f'z = {fix_z:.1f}')
...
for ifix_logM, fix_logM in enumerate(uniq_logM):
    ...
    axs[1].errorbar(z[mask], y[mask], yerr=[sketchy_factor*np.abs(ymin[mask]-y[mask]), 
                                            sketchy_factor*np.abs(ymax[mask]-y[mask])],
                    fmt='.', c=f'C{ifix_logM}'# c=cmap2(ifix_logM/len(uniq_logM))
                    )
    # Plot the fitted line
    axs[1].plot(z_th, y_th, c=f'C{ifix_logM}', # c=cmap2(ifix_logM/len(uniq_logM)), 
...
# Save 
save_dir = f'result/fitting_full/Nboots_{args.Nboots}/'
