...
parser.add_argument('--zmax',    default=2,   type=int)
...
fig, axs = plt.subplots(1, 1, figsize = (6, 5), dpi=600)
...
axins = inset_axes(axs, width="40%", height="40%", loc="upper right", borderpad=2)
...
for b in uniq_bins:
    ...
    # Plot SORTED zoomed-in data
    zoomed_idx = np.where(current_z <= args.zmax)
    if len(zoomed_idx[0]) > 0:
        
        current_z = current_z[zoomed_idx]
        current_data = current_data[zoomed_idx]
        print('Redshifts for current mass cut: ', current_z)
        
        sorted_idx = np.argsort(current_z)
        axins.plot(current_z[sorted_idx], current_data[:, feat_idx, 1][sorted_idx], color=cmap(norm(b)), alpha=0.2)
        axins.errorbar(current_z, current_data[:, feat_idx, 1], 
                    yerr=[current_data[:, feat_idx, 1]-current_data[:, feat_idx, 0], 
                        current_data[:, feat_idx, 2]-current_data[:, feat_idx, 1]],
                    fmt='.', color=cmap(norm(b))
                        )
    
# Add lines to connect the inset plot to the main plot
...
plt.savefig(os.path.join(save_dir, f'{args.DM}_{feats[feat_idx]}_vs_{args.z_or_a}-zoom-in-zmax-{args.zmax}'))
plt.close()