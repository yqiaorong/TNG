...
parser.add_argument('--Nsample',  default=2000,type=int)
...
halos_list = [f'bin-{int(item*10)}-{int(item*10+5)}.npy' for item in mass_bins[:-1]]
print(halos_list)
    ...
    if all(x > 1 for x in num_halos_per_bin):
        ...
        for i in range(num_bins):
            ...
            plot_profile(R200_median, radius, rho, rho_err, slope, slope_err, 
                        ...
                        f'result/bootstrap/{args.sim}/snap_{args.snapnum}', f'boots_{valid_boots}',
                        save_data=True)
...
# Save the result
save_dir = f'result/bootstrap_stats/{args.sim}/'
...
np.save(save_dir+f'snap_{args.snapnum}_Rsp_stats', save_data)