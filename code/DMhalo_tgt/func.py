...
def stacked_density_profile(radii_2d_array, densities_2d_array, M200_1d_array, R200_1d_array,
    ...
    # Compute median R200
    radius200_median = np.median(radius200_list) # [cMpc/h]
    del radius200_list

    ### Get rid of gravitationally softening length part ###
    
    gsl = 2.5 * 12 * 0.001 # gravitational softening length [cMpc/h]
    radii_phys = radius200_median * radii_dimless # The median radii bins in cMpc/h

    # The index in radius bins which is greater than gsl [cMpc/h]
    gsl_radius_idx = next((ir for ir, r in enumerate(radii_phys) if r >= gsl), -1) 
    
    radii_dimless = radii_dimless[gsl_radius_idx:] # The median radii bins dimensionless
    print(radii_dimless.shape)
    del radii_phys
    ...
    # Remove the gravitational softening length part
    select_densities_array = select_densities_array[gsl_radius_idx:]
    print(select_densities_array.shape)
    del densities_list
    ...
    if use_bootstrap == True:
        ...
        medians = np.percentile(select_densities_array, 50, axis=1) # shape: (N radii, 1)
        errors = np.percentile(select_densities_array, [16,84], axis=1).T # shape: (N radii, 2)
...
def fit_profile_parametric(bin_centers, densities, density_errors, R_200_mean):
    ...
    evaluate_profile_at_edges = np.logspace(np.log10(bin_centers[0]), np.log10(bin_centers[-1]), 1024)
