...
# HaloIdx mass
haloMass_df = get_halo_feat_at_snapX(basePath, haloIdx_df, 'Group_M_Mean200')
snap_haloMass = np.array(haloMass_df.loc['snap_99'])
print(snap_haloMass.shape)
...
select_rows = abs_diff.idxmin(skipna=False)
...
for fname in halos_fnames:
    ...
    data['crossz'] = crossing_z
    print(data['crossz'].shape, data['halo_M_Mean200'].shape)
    print(data.keys())
