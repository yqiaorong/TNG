""""This script plots depth (top panel) and width (bottom panel) as a function of redshift for a few mass cuts.
...
for b in uniq_bins:
    ...
    # Plot depth
    axs[0].plot(current_z[sorted_idx], current_data[:, 1, 1][sorted_idx], color=cmap(norm(10**b)), 
                lw=1, alpha=0.5) 
    ...
    # Plot width
    axs[1].plot(current_z[sorted_idx], current_data[:, 2, 1][sorted_idx], color=cmap(norm(10**b)), 
                lw=1, alpha=0.5) 
